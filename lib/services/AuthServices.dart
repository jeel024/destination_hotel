...
Future<HotelModel> signInWithEmail(String email, String password) async {
  if (await hotelService.isUserExist(email, LoginTypeApp)) {
    ...
    if (userCredential.user != null) {
      ...
      return await hotelService.userByEmail(user.email).then((value) async {
        hotelModel = value;
        ...
        //
        await setUserDetailPreference(hotelModel);
...
Future<void> setUserDetailPreference(HotelModel hotelModel) async {
  await setValue(USER_ID, hotelModel.id);
  await setValue(USER_NAME, hotelModel.name);
  await setValue(USER_EMAIL, hotelModel.email);
  await setValue(USER_PROFILE, hotelModel.image.validate());
