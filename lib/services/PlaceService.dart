...
class PlaceService extends BaseService {
  ...
  Future<List<PlaceModel>> fetchPlaceList({required List<PlaceModel> list, String? catId, String? stateId,String? cityId, bool isPopular = false}) async {
    ...
    if (catId != null) {
      ...
      query = ref!.where(CommonKeys.status, isEqualTo: 1).where(PlaceKeys.stateId, isEqualTo: cityId).orderBy(CommonKeys.createdAt, descending: true);
    }else if (cityId != null) {
  ...
  Future<List<PlaceModel>> getPlaces() async {
      Query query = ref!.where(CommonKeys.status, isEqualTo: 1).orderBy(PlaceKeys.name, descending: true);
